...
class ScrollLabel(Widget):
    def __init__(self, id, title: str, grid: 'py_cui.grid.Grid', row: int, column: int, row_span: int, column_span: int, padx: int, pady: int, status: str, logger: 'py_cui.debug.PyCUILogger'):
        """Initializer for scroll label. calls superclass initialzers and sets status text
        """

        Widget.__init__(self, id, title, grid, row, column, row_span, column_span, padx, pady, logger)
        py_cui.ui.Menu_Implementation.__init__(self, logger)
        self.set_help_text(status)
    ...
    def _handle_key_press(self, key_pressed: int) -> None:
        ...
        # TODO this is for selecting things? CHECK _PROCESS_SELECTION_CHANGE_EVENT
        if self.get_selected_item_index() != current and self._on_selection_change is not None:
            self._process_selection_change_event()


    def _draw(self) -> None:
        """ Overrides ase class draw function
        """

        Widget._draw(self)
        # TODO How to set custom colour?
        self._renderer.set_color_mode(self._color)
        ...
        for item in self._view_items:
            ...
            if line_counter < self._top_view:
                ...
                if line_counter == self._selected_item:
                    # TODO: This is what sets a specific line to be highlighted,
                    #       Pass a list of URL locations to the constructor so that it can use it when
                    #       Rendering. Selecting text should not be default behaviour for this.
                    self._render.draw_text(self, line, self._start_y_ + counter, selected=True)
                else:
